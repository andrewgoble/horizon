...
- **Cart-drawer upsell is hand-built, not an app.** `sections/cart-upsells.liquid` + `assets/cart-upsells.js`, pulling from Shopify's Section Rendering / Recommendations API. Fixed three times: S32 (2026-07-02, module-level cache + observedAttributes), a race-condition fix on 2026-07-17 (commit d406960 — `attributeChangedCallback` firing on initial element upgrade raced `connectedCallback` via a shared AbortController), and a morph-wipe fix on 2026-07-20 (commit 6696b10 — Horizon's `morphSection()` deletes the client-rendered carousel on every cart re-render since the server template renders it empty; fixed with `data-skip-subtree-update` on `<cart-upsells>`, `snippets/cart-drawer.liquid`). Full write-up: `learnings-shopify.md` under "Horizon Custom Element Cache."
